...
// 由库容反查水位：按同一条分段曲线反解，与 capacityAt 对得上
...
// 汛期判断：按 月-日 与起止日期比较（含两端），不能只看月份
function inFloodSeason(dateStr, settings) {
  const parts = String(dateStr || '').split('-');
  if (parts.length !== 3) return false;
  const monthDay = String(parts[1]).padStart(2, '0') + '-' + String(parts[2]).padStart(2, '0');
  return monthDay >= String(settings.floodSeasonStart) && monthDay <= String(settings.floodSeasonEnd);
}
